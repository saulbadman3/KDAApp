...
class ResearchPage(ctk.CTkFrame):
    ...
    def _build(self):
        section_title(self, "Research & Analysis").pack(anchor="w", pady=(0, 4))
        ...
        lbl(uc, "Target User Configuration", 12, C_ACCENT, bold=True).pack(anchor="w", padx=20, pady=(10, 4))
        ...
        self.username_entry = ctk.CTkEntry(uc, height=46, corner_radius=10, border_color=C_BORDER, fg_color=C_PANEL, font=ctk.CTkFont("Segoe UI", 16),
                        placeholder_text="Type username to load context...", placeholder_text_color=C_MUTED)
        ...
        self.entry.configure(show="•", placeholder_text="Type target password...")
        ...
        r1.pack(fill="x", padx=20, pady=(12, 6))
        ...
        btn(br, "Clear Session", self._clear, C_DANGER, w=110).pack(side="left")

        lc = card(self)
        lc.pack(fill="both", expand=True)
        lbl(lc, "Statistical Evaluation Summary", 11, C_MUTED).pack(anchor="w", padx=16, pady=(12, 4))
    ...
    def _submit(self):
        ...
        if self.app.profile is None:
            messagebox.showwarning("No Profile Loaded", f"Please train or load a biometrical profile for '{username}' first.")
            self.entry.reset()
            return
        ...
        if not self._target_password:
            ...
            self._log_msg(f"--- Research Session started for '{username}' ---")

        if typed_password != self._target_password:
            self.status.configure(text="Text mismatch. Enter the exact password of the target user.", text_color=C_DANGER)
            self.entry.reset()
            return
        ...
        if self.mode_var.get() == "Legitimate":
            ...
            self.status.configure(text=f"Legit score recorded: {sc:.4f}", text_color=C_SUCCESS)
        else:
            self.impostor_scores.append(sc)
            self.imp_badge.configure(text=f"Impostor: {len(self.impostor_scores)}")
            self.status.configure(text=f"Impostor score recorded: {sc:.4f}", text_color="#f87171")
    ...
    def _compute(self):
        ...
        lines = ["─"*48,
                 f"Target Account: {self.username_entry.get().strip()}",
                 f"α = {alpha:.3f} -> Statistical Threshold = {thr:.4f}",
