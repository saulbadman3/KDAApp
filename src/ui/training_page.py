...
class TrainingPage(ctk.CTkFrame):
    ...
    def _submit(self):
        """Processes each password entry, validating consistency and extracting mathematical keystroke features."""
        ...
        # Lock session to this specific password structure on the first successful verification
        if not self._target_password:
            if not store.verify(username, typed_password):
                self.status.configure(text="Training failed: Invalid username or password.", text_color=C_DANGER)
                self.entry.reset()
                return
        ...
        # Reject entry if text changed during the current training session
        ...
        # Dynamic extraction: Dwell time per key and Flight time between keystrokes
        ...
        flight = np.maximum(flight, 0) # Mitigates overlapping key metrics
    ...
    def _save(self):
        """Compiles the accumulated attempt vectors into a statistical biometric matrix template."""
        username = self.username_entry.get().strip()
        ...
        # Evaluate and log structural stability of features (Coefficient of Variation)
        ...
        path = filedialog.asksaveasfilename(defaultextension=".kda", filetypes=[("KDA Profile","*.kda")], initialfile=suggested_filename)
    ...
    def _clear(self):
        """Flushes session attempts and resets UI elements for a new dataset generation."""
        self.attempts.clear()
        self._target_password = None
